...
DEFAULT_DEPLOYMENT_REVISION_NAME="current"
DEFAULT_DEPLOYMENT_REVISION_NUMBER="`date +%Y%m%d%H%M%S`"
CURRENT_DEPLOYMENT_REVISION_NAME="${CIRCLE_TAG:-$CIRCLE_BRANCH}"
CURRENT_DEPLOYMENT_REVISION_NAME="${CURRENT_DEPLOYMENT_REVISION_NAME//[\/]}"
CURRENT_DEPLOYMENT_REVISION_NUMBER="${CIRCLE_SHA1:-$CIRCLE_BUILD_NUM}"
CURRENT_DEPLOYMENT_REVISION_NAME="${CURRENT_DEPLOYMENT_REVISION_NAME:-DEFAULT_DEPLOYMENT_REVISION_NAME}"
CURRENT_DEPLOYMENT_REVISION_NUMBER="${CURRENT_DEPLOYMENT_REVISION_NUMBER:-$DEFAULT_DEPLOYMENT_REVISION_NUMBER}"
CURRENT_DEPLOYMENT_REVISION_DESCRIPTION="${CURRENT_DEPLOYMENT_REVISION_NAME}-${CURRENT_DEPLOYMENT_REVISION_NUMBER}"
echo "CURRENT_DEPLOYMENT_REVISION_NAME: ${CURRENT_DEPLOYMENT_REVISION_NAME}"
echo "CURRENT_DEPLOYMENT_REVISION_NUMBER: ${CURRENT_DEPLOYMENT_REVISION_NUMBER}"
echo "CURRENT_DEPLOYMENT_REVISION_DESCRIPTION: ${CURRENT_DEPLOYMENT_REVISION_DESCRIPTION}"

export AWS_CODE_DEPLOY_REGION="us-east-1"
export AWS_CODE_DEPLOY_APPLICATION_NAME="arbiter"
export AWS_CODE_DEPLOY_S3_BUCKET="th-deployments"
export AWS_CODE_DEPLOY_APP_SOURCE="./"
export AWS_CODE_DEPLOY_S3_SSE=true
export AWS_CODE_DEPLOY_S3_LIMIT_BUCKET_FILES=3
export AWS_CODE_DEPLOY_DEPLOYMENT_DESCRIPTION="Deployed via CircleCI on $(date)"
export AWS_CODE_DEPLOY_REVISION_DESCRIPTION="${CURRENT_DEPLOYMENT_REVISION_NAME} (#${CURRENT_DEPLOYMENT_REVISION_NUMBER})"
export AWS_CODE_DEPLOY_MINIMUM_HEALTHY_HOSTS=1
export AWS_CODE_DEPLOY_DEPLOYMENT_CONFIG_NAME="CodeDeployDefault.OneAtATime"
export AWS_CODE_DEPLOY_SERVICE_ROLE_ARN="arn:aws:iam::695135537037:role/CodeDeployServiceRole"
export AWS_CODE_DEPLOY_S3_FILENAME="${AWS_CODE_DEPLOY_APPLICATION_NAME}-${AWS_CODE_DEPLOY_DEPLOYMENT_GROUP_NAME}-${CURRENT_DEPLOYMENT_REVISION_DESCRIPTION}"
...
installJq() {
  runCommand "sudo apt-get install -y jq"
  success "Installing Bash Json parser client, jq, succeeded"
}
...
if [ $? -ne 0 ]; then
  error "$APPLICATION_EXISTS_OUTPUT"
  exit 1
...
if [ $? -ne 0 ]; then
  error "$DEPLOYMENT_CONFIG_EXISTS_OUTPUT"
  exit 1
...
DEPLOYMENT_READY_OPT="deploymentReadyOption={actionOnTimeout='CONTINUE_DEPLOYMENT',waitTimeInMinutes=0}"
TERMINATION_OPT="terminateBlueInstancesOnDeploymentSuccess={action='TERMINATE',terminationWaitTimeInMinutes=0}"

if [ $? -ne 0 ]; then
  error "$DEPLOYMENT_GROUP_EXISTS_OUTPUT"
  exit 1
  ...
  # h1 "Step 5.1: Updating Deployment Group to terminate original instances immediately"
  # AUTO_SCALING_GROUPS_CONFIG="--auto-scaling-groups $AUTO_SCALING_GROUPS"
  # BLUE_GREEN_DEPLOYMENT_CONFIG="--blue-green-deployment-configuration $TERMINATION_OPT,$DEPLOYMENT_READY_OPT"
  # UPDATE_DEPLOYMENT_CONFIG_CMD="aws deploy update-deployment-group --application-name $APPLICATION_NAME --current-deployment-group-name $DEPLOYMENT_GROUP $BLUE_GREEN_DEPLOYMENT_CONFIG"
  # info "$UPDATE_DEPLOYMENT_CONFIG_CMD"
  # UPDATE_DEPLOYMENT_GROUP_OUTPUT=$($UPDATE_DEPLOYMENT_CONFIG_CMD 2>&1)
  # if [ $? -ne 0 ]; then
  #   error "$UPDATE_DEPLOYMENT_GROUP_OUTPUT"
  # else
...
if [ "true" = "$DEPLOYMENT_OVERVIEW" ]; then
  ...
  while :
    do
      ...
      # Deployment Overview
      IN_PROGRESS=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.deploymentOverview.InProgress?")
      PENDING=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.deploymentOverview.Pending?")
      SKIPPED=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.deploymentOverview.Skipped?")
      SUCCEEDED=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.deploymentOverview.Succeeded?")
      FAILED=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.deploymentOverview.Failed?")

      # Deployment Status
      STATUS=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.status")
      IS_TERMINATING_WAITING=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.instanceTerminationWaitTimeStarted")

      IS_PROVISIONING=1
      if [ "$IN_PROGRESS" = "null" ]; then IN_PROGRESS="-"; else IS_PROVISIONING=0; fi
      if [ "$PENDING" = "null"     ]; then PENDING="-";     else IS_PROVISIONING=0; fi
      if [ "$SKIPPED" = "null"     ]; then SKIPPED="-";     else IS_PROVISIONING=0; fi
      if [ "$SUCCEEDED" = "null"   ]; then SUCCEEDED="-";   else IS_PROVISIONING=0; fi
      if [ "$FAILED" = "null"      ]; then FAILED="-";      else IS_PROVISIONING=0; fi

      if (( $IS_PROVISIONING == 1 )); then
        NEW_OUTPUT="${bold}${blink}Status${reset}  | Provisioning...\n"
      else
        NEW_OUTPUT="${bold}${blink}Status${reset} | Pending: $PENDING  | In Progress: $IN_PROGRESS  | Skipped: $SKIPPED  | Succeeded: $SUCCEEDED  | Failed: $FAILED  |\n"
      fi
      ...
      if [ "$STATUS" = '"Failed"' ]; then
        ERROR_CODE=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.errorInformation.code?")
        ERROR_MESSAGE=$(echo "$DEPLOYMENT_GET_OUTPUT" | jsonValue "deploymentInfo.errorInformation.message?")
        printf "${bold}Status${reset}  | In Progress: $IN_PROGRESS  | Pending: $PENDING  | Skipped: $SKIPPED  | Succeeded: $SUCCEEDED  | Failed: $FAILED  |\n"
        error "Deployment failed: ($ERROR_CODE) $ERROR_MESSAGE"
        ...
        AUTO_SCALING_GROUP="CodeDeploy_${DEPLOYMENT_GROUP}_${DEPLOYMENT_ID}"
        CLEANUP_OUTPUT=""
        printf "Attempting to cleanup deployment $DEPLOYMENT_ID...\n"
        DEPLOYMENT_CMD="aws autoscaling delete-auto-scaling-group --auto-scaling-group-name $AUTO_SCALING_GROUP --force-delete"
        runCommand "$DEPLOYMENT_CMD" \
                   "Cleanup of deployment \"$DEPLOYMENT_ID\" failed" \
                   "" \
                   CLEANUP_OUTPUT
        printf "$CLEANUP_OUTPUT\n"
        exit 1
      fi
      ...
      if [ "$IS_TERMINATING_WAITING" = "true" ] && [ $IS_TERMINATING != 1 ]; then
        IS_TERMINATING=1
        TERMINATE_IMMEIDATELY_COMMAND="aws deploy skip-wait-time-for-instance-termination --deployment-id $DEPLOYMENT_ID"
        h2 "Terminating original instances"
        info "$TERMINATE_IMMEIDATELY_COMMAND"
        TERMINATION_OUTPUT="$(eval $TERMINATE_IMMEIDATELY_COMMAND 2>&1)"
        if [ $? != 0 ]; then
          warnError "$TERMINATION_OUTPUT"
          error "Termination of original instances failed. Please terminate manually. The build process will wait until termination is complete..."
        fi
      fi
      sleep 2
    done
